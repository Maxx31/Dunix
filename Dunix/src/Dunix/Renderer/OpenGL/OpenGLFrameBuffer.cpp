...
namespace Dunix
{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		if (m_ColorAttachmentSpecifications.size())
		{
			...
			for (size_t i = 0; i < m_ColorAttachments.size(); i++)
			{
				...
				case FramebufferTextureFormat::RED_INTEGER:
					Utils::AttachColorTexture(m_ColorAttachments[i], m_Specification.Samples, GL_R32I, GL_RED_INTEGER, GL_INT, m_Specification.Width, m_Specification.Height, static_cast<int>(i));
					break;
				}
			}
		}
		...
		if (m_ColorAttachments.size() > 1)
		{
			...
			{
				DX_CORE_WARN("Framebuffer supports up to 4 color attachments.");
				return;
			}
		...
		{
			DX_CORE_WARN("Framebuffer is incomplete!");
		}
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			DX_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
	{
		...
		{
			DX_CORE_WARN("Framebuffer attachment index {0} is out of range.", attachmentIndex);
			return -1;
		}
